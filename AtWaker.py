...
def rate_calc(db,dt):
    ...
    S=I/sum([np.log((i+1)/N*100)*R**(i+1) for i in range(N)])*sum([R**((i+1)/N*100) for i in range(N)])
    print(R,S)
    if len(dbr)>0:
        vlast=dbr.iloc[-1]
        dbr.loc[dt]=vlast
    ...
    for xx in db.columns:
        ...
        if True:
            ...
            vperfext=np.array(sorted([vperf[i//N]-S*np.log((i+1)/N*100) for i in range(len(vperf)*N)])[::-1])
            print(vperfext)
            ratenom=0
            rateden=0
            if len(vperfext)>=N:
